...
using an extremely simple Neural Network, just one hidden layer.
...
"""
We classify the users in FILE_USERS using the data in all_tweets.txt via a basic Neural Network algorithm.
...
*epsilon
*EPOCHS
*number_hidden_layer (this is the number of neurons in the (unique) hidden layer)
...
def main():
    ...
    epsilon=0.03 #regularization lambda
    EPOCHS=20
    number_hidden_layer=50 #this is the number of neurons in the (unique) hidden layer
    ...
    print("-----------------------------")
    print("METHOD - NEURAL NETWORK with ONE HIDDEN LAYER")
    print("-----------------------------")


    print("Hello,\nwe will use a basic Neural Network with only one hidden layer\nto classify twitter users\n")
    setpath()
    ...
    model_bnn.add(tf.keras.layers.Dense(number_hidden_layer, kernel_regularizer=regularizers.l2(epsilon), bias_regularizer=regularizers.l2(epsilon), activation='relu'))
    ...
    if os.path.isfile(FOLDER+"/update_1lNN"+str(n_features)+"_"+str(U)+".txt")==True:
        with open(FOLDER+"/update_1lNN"+str(n_features)+"_"+str(U)+".txt") as h:
            update=h.read()
            h.close()
        ...
        with open(FOLDER+"/X_train_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy",'wb') as f:
            pickle.dump(X_train,f)


        file=FOLDER+"/X_train_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_train,f)

        file=FOLDER+"/Y_train_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_train,f)

        file=FOLDER+"/X_dev_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_dev,f)

        file=FOLDER+"/Y_dev_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_dev,f)

        file=FOLDER+"/X_test_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_test,f)

        file=FOLDER+"/Y_test_politic"+today_string+"_1lNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_test,f)

        with open(FOLDER+"/update_1lNN"+str(D)+"_"+str(U)+".txt", "w") as h:
            h.write(today_string+"_1lNN"+str(D)+"_"+str(U))
            h.close()
    ...
    print("The # of neurons (relu) in the hidden layer is", number_hidden_layer)
    print("\n")
